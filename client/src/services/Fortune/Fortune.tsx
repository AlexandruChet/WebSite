...
const Fortune: React.FC = () => {
  ...
  const spinWheel = () => {
    ...
    const randomDegree = 3600 + randomIndex * sectorAngle + sectorAngle / 2;
    ...
    setTimeout(() => setSelectedValue(sectors[randomIndex].value), 4000);
  };
      ...
      <div className="fortune">
        ...
        <svg ref={wheelRef} width="320" height="320" className="wheel">
          {sectors.map((sector, i) => {
            ...
            const textX =
              center + (radius / 2) * Math.cos((-textAngle * Math.PI) / 180);
            const textY =
              center + (radius / 2) * Math.sin((-textAngle * Math.PI) / 180);
            return (
              <g key={i}>
                <path
                  d={path}
